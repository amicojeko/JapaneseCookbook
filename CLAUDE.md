...
- `mcp/` — **standalone** MCP server (own `package.json`, published to npm as `mcp-paginegiappe`). Thin proxy: its tools call the deployed Netlify functions, so data is always live — the package rarely needs republishing (only when tools/params change). See `mcp/README.md`.

**When you add a tool or change its params**, bump `mcp/package.json` version and `npm publish` from `mcp/`.
