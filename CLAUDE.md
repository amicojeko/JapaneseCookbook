...
- `netlify/functions/` — serverless API (esbuild-bundled; they `require()` the knowledge JSON):
  ...
  - `mcp.mjs` — **hosted MCP server** (Netlify Function v2, served at `https://paginegiappe.it/mcp`). Streamable HTTP, stateless, web-standard transport (`WebStandardStreamableHTTPServerTransport`), JSON response mode, **no auth** (public read-only). Add as a Custom Connector in Claude.ai/Desktop with that URL — no install needed. Imports the tool definitions from `netlify/lib/mcp-tools.mjs`.
- `netlify/lib/mcp-tools.mjs` — `createServer()` factory: the 8 MCP tools, the Markdown formatters (foto + schede ricetta) and the directive server `instructions`. Single source of truth for the MCP tools. Lives outside `netlify/functions/` so it isn't published as its own function. Requires `@modelcontextprotocol/sdk` + `zod` in the **root** `package.json` (so Netlify installs them and esbuild bundles the function). **Dual output:** every tool returns Markdown `content` (for non-Apps clients: Claude connector, Gemini CLI, MCP Inspector) AND — via `registerTool` with an `outputSchema` — a validated `structuredContent` object. With an `outputSchema` declared, the handler MUST return `structuredContent` on success (errors are exempt); schemas are intentionally lenient (optional/nullable) and validation does not strip extra keys.
- `static/openapi.json` — OpenAPI 3.1 spec for GPT Custom Actions (import URL: `https://paginegiappe.it/openapi.json`).

**ChatGPT App (Apps SDK) — UI widgets.** Three tools (`find_recipes`, `suggest_menu`, `find_shops`) carry `_meta["openai/outputTemplate"]` → a `ui://widget/<name>.html` resource (mimetype `text/html;profile=mcp-app`) so ChatGPT renders a native widget instead of plain text. Pipeline:
- `widgets/*.tsx` — Preact widget sources (NOT compiled by Docusaurus or `tsc`; excluded in `tsconfig.json`). `_shared.tsx` holds the `useToolOutput()` hook + `RecipeCard`. Widgets read `window.openai.toolOutput` (the tool's `structuredContent`).
- `scripts/build-widgets.js` — esbuild bundles each widget to a self-contained HTML string → `netlify/lib/widgets/<name>.html.mjs` (`export default`). Runs in the `prebuild` chain (and `npm run build-widgets`). **Must pass `tsconfigRaw` to force the Preact JSX runtime** — the repo `tsconfig.json` has `jsx:"react"` which esbuild would otherwise inherit (→ `React is not defined`). The generated `*.html.mjs` are committed (like other generated artifacts); `mcp-tools.mjs` imports them statically so Netlify's esbuild inlines them into the `/mcp` function.
- The map widget loads CartoDB tiles; its tool sets `_meta.ui.csp` (`CSP_MAP`) to allow `*.basemaps.cartocdn.com`. Image widgets allow `paginegiappe.it` (`CSP_IMG`).
- **Testing (Developer Mode):** ChatGPT → Settings → Apps & Connectors → Advanced → Developer Mode; add a connector pointing at `https://paginegiappe.it/mcp` (or an ngrok tunnel of `netlify dev`). Validate first with `npx @modelcontextprotocol/inspector`. See `CHATGPT_APP.md` for the full runbook (incl. Store submission).

**When you change an MCP tool or its params**, edit `netlify/lib/mcp-tools.mjs`; the hosted endpoint updates automatically on the next Netlify deploy. (There is no longer a standalone npm package — the MCP is hosted-only.)
