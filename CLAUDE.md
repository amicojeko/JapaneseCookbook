...
**Author signature at the end of every post** — the last line (after any closing paragraph) must be a dash followed by a link to the author's page:
```
— [Nome Autore](/blog/authors/author-slug)
```
Use the author's `name` field from `blog/authors.yml` and the key as slug. Example: `— [ToolsReborn – Wabi Sabi Story](/blog/authors/toolsreborn)`. Check an existing post (e.g. `blog/2026-06-07-hyorogan/index.md`) if in doubt.
...
- `netlify/lib/mcp-tools.mjs` — `createServer()` factory: the 8 MCP tools, the Markdown formatters (foto + schede ricetta) and the directive server `instructions`. Single source of truth for the MCP tools. Lives outside `netlify/functions/` so it isn't published as its own function. Requires `@modelcontextprotocol/sdk` + `zod` in the **root** `package.json` (so Netlify installs them and esbuild bundles the function).
